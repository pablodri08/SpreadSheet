...
export class AppComponent implements OnInit{
  ...
  // Evalúa si se ingresó una fórmula
    ...
    try {
      if (value){
        ...
        if ((input) && (input.length > 3)) {
          ...
          const rest = input.slice(1, input.length);
          switch (first) {
            case '=':
  ...
  // Retorna el valor numérico de cada operando
    ...
    for (let i = 0; i < length; i++) {
      ...
      if (cellRegExp.test(values[i])) {
        ...
        if ((row === rowOnChange) && (col === colOnChange)) {
          throw new Error('La celda actual no tiene un valor definido!');
        } else {
